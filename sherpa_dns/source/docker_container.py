...
class DockerContainerSource:
    ...
    def __init__(self, label_prefix: str = "sherpa.dns", label_filter: Optional[str] = None):
        """
        ...
        try:
            ...
            try:
                ...
                self.docker_client.ping()
                self.logger.debug("Successfully connected to Docker daemon with explicit socket path")
            except docker.errors.DockerException as inner_e:
                self.logger.error(f"Error connecting to Docker daemon with explicit socket path: {inner_e}")
    ...
    async def endpoints(self) -> List[Endpoint]:
        ...
        try:
            ...
            for container in containers:
                ...
                if self.label_filter and not self._matches_filter(container.labels, self.label_filter):
                    continue
    ...
    async def _reconnect_docker_client(self) -> bool:
        ...
        try:
            ...
            self.logger.warning("Failed reconnecting with default, trying explicit socket path")
            try:
                # Try explicit path
                self.docker_client = docker.DockerClient(base_url="unix://var/run/docker.sock")
                self.docker_client.ping()
                self.logger.debug("Successfully reconnected to Docker daemon with explicit socket path")
                return True
            except docker.errors.DockerException as e:
                self.logger.error(f"Error reconnecting to Docker daemon with explicit socket path: {e}")
    ...
    def _blocking_event_listener(self, loop: asyncio.AbstractEventLoop):
        ...
        try:
            if self.docker_client is None:
                 self.logger.error("Event listener thread: Docker client is None. Cannot start.")
                 return

            self.logger.debug("Event listener thread polling Docker API for events.")
            # Note: events() is blocking when iterated
            events = self.docker_client.events(decode=True, filters={"type": "container"}) 
            for event in events:
                event_type = event.get("status")
                if event_type in ["start", "die", "stop", "kill", "pause", "unpause"]: # Added more events just in case
                    # Put event onto the asyncio queue from the thread
                    future = asyncio.run_coroutine_threadsafe(self.event_queue.put(event), loop)
                    try:
                        # Wait briefly for the put to complete to avoid overwhelming queue on burst
                        future.result(timeout=5) 
                        self.logger.debug(f"Event listener thread: Queued event {event_type} - {event.get('id', '')[:12]}")
                    except (concurrent.futures.TimeoutError, asyncio.TimeoutError):
                         self.logger.warning("Event listener thread: Timeout waiting for event queue put.")
                    except Exception as e:
                         # Catch specific exceptions if possible, e.g., queue full?
                         self.logger.error(f"Event listener thread: Error getting result from queue put: {e}")
                else:
             ...
             self.logger.error(f"Event listener thread: Docker APIError: {e}. Listener stopping.")
             # Consider resetting docker_client to None here to force reconnect attempt
             self.docker_client = None 
        except docker.errors.DockerException as e:
             # Catch other docker exceptions
            self.logger.error(f"Event listener thread: DockerException: {e}. Listener stopping.")
            self.docker_client = None
        except Exception as e:
             # Catch any other unexpected errors in the thread
            self.logger.exception(f"Event listener thread: Unexpected error: {e}. Listener stopping.")
        finally:
    ...
    async def watch_events(self) -> None:
        ...
        """
        self.logger.debug("Docker source event watcher task started (using thread executor).")
        
        while True: # Keep trying to run the listener thread
            if not await self._reconnect_docker_client():
                self.logger.info("Event watcher task: Failed to connect to Docker. Retrying in 10 seconds...")
            ...
            try:
                ...
                listener_future = loop.run_in_executor(None, self._blocking_event_listener, loop)
                ...
                self.logger.warning("Event listener thread exited. Will attempt restart after delay.")
                ...
                if self.docker_client is not None:
                    try:
                        ...
                        self.logger.info("Docker connection seems lost, resetting client.")
                        self.docker_client = None
                 ...
                 self.logger.exception(f"Error running or awaiting event listener thread: {e}")
                 # Consider if docker_client needs reset here too
                 self.docker_client = None 
                 
            # Wait before restarting the listener attempt
            self.logger.info("Waiting 10 seconds before attempting to restart listener thread...")
            await asyncio.sleep(10) 
    ...
    def _endpoints_from_container(self, container: Container) -> List[Endpoint]:
        ...
        for hostname in hostnames:
            ...
            proxied_str = self._get_label_value(container_labels, hostname, "proxied", "false")
            target = self._get_label_value(container_labels, hostname, "target", None)
            network_name = self._get_label_value(container_labels, hostname, "network", None) # Get network name from label
            ...
            if target:
                ...
                container_ip = self._get_container_ip(container, network_name) # Pass network_name
                if container_ip:
                    ...
                    elif record_type == "AAAA" and isinstance(ip_obj, ipaddress.IPv6Address):
                         targets = [container_ip]
                    else:
                        self.logger.warning(f"IP address {container_ip} type mismatch for record type {record_type} on container {container_name}. Skipping.")
                ...
                targets = [f"{container_name}"] # Default CNAME target to container name
            ...
            if targets:
                ...
                self.logger.warning(f"No suitable target found for hostname {hostname} (Type: {record_type}, Network: {network_name or 'default'}) in container {container_name} ({container_id[:12]})")
    ...
    def _get_hostnames_from_labels(self, labels: Dict[str, str]) -> Set[str]:
        ...
        if hostname_label in labels:
            ...
            names = [name.strip() for name in labels[hostname_label].split(',') if name.strip()]
        ...
        if not hostnames:
            self.logger.debug("No hostnames found in labels using prefix %s", self.label_prefix)
            
        return hostnames

    def _get_label_value(self, labels: Dict[str, str], hostname: str, key: str, default: Optional[str] = None) -> Optional[str]:
        """
        ...
        for label, value in labels.items():
             if label.startswith(hostname_alias_prefix):
                  ...
                  defined_hostnames = {name.strip() for name in value.split(',') if name.strip()}
                  if hostname in defined_hostnames:
                      alias_key = label[len(hostname_alias_prefix):] # Extract the alias (e.g., 'web')
    ...
    def _get_container_ip(self, container: Container, network_name: Optional[str] = None) -> Optional[str]:
        """
        ...
        try:
            ...
            if not networks:
                self.logger.warning(f"No network settings found for container {container.name}")
                return None
            ...
            if network_name:
                if network_name in networks:
                    ...
                    self.logger.warning(f"Network '{network_name}' not found for container {container.name}. Available: {list(networks.keys())}")
                    return None
            elif len(networks) == 1:
                ...
                if 'bridge' in networks:
                     ...
                     self.logger.debug(f"Multiple networks found for {container.name}, using default 'bridge'. Specify label '{self.label_prefix}/network' if needed.")
                else:
                     first_network_name = sorted(networks.keys())[0]
                     target_network = networks[first_network_name]
                     self.logger.debug(f"Multiple networks found for {container.name}, using first network '{first_network_name}'. Specify label '{self.label_prefix}/network' if needed.")

            if target_network:
                ip_address = target_network.get('IPAddress')
                if ip_address and self._is_valid_ip(ip_address): # Ensure it's a valid, non-empty IP
                    return ip_address
                else:
                    ...
                    if ip_address_v6 and self._is_valid_ip(ip_address_v6):
                         ...
                         self.logger.warning(f"No valid IP address found for container {container.name} on network {network_name or 'selected network'}. Network details: {target_network}")

        except docker.errors.NotFound:
            self.logger.error(f"Container {container.name} not found during IP address retrieval.")
        except Exception as e:
            self.logger.exception(f"Error getting IP address for container {container.name}: {e}")
