...
class CloudflareProvider:
    ...
    async def zones(self) -> List[Dict[str, str]]:
        ...
        try:
            ...
            # Filter zones
            filtered_zones = []
            self.logger.debug(f"Filtering zones based on domain_filter: {self.domain_filter} and exclude_domains: {self.exclude_domains}")
            for zone in zones:
                # Use attribute access
                zone_name = getattr(zone, "name", None)
                zone_id = getattr(zone, "id", None)
                if not zone_name or not zone_id:
                    self.logger.warning(f"Skipping zone object due to missing name or id: {zone}")
                    continue

                self.logger.debug(f"Processing zone: Name='{zone_name}', ID='{zone_id}'")

                # Check if zone should be excluded
                if self._matches_domain_filter(zone_name, self.exclude_domains):
                    self.logger.debug(f"Zone '{zone_name}' is in exclude_domains, skipping.")
                    continue
                
                # Check if zone is in include filter (if filter is defined)
                if self.domain_filter and not self._matches_domain_filter(zone_name, self.domain_filter):
                    self.logger.debug(f"Zone '{zone_name}' is not in domain_filter, skipping.")
                    continue

                self.logger.debug(f"Zone '{zone_name}' passed filters. Adding to managed zones.")
            ...
            self.logger.debug(f"Finished filtering. Found {len(filtered_zones)} managed zones.")
            return filtered_zones
        # Use cloudflare.APIError for specific API errors
        except cloudflare.APIError as e:
            error_code = getattr(e, 'code', 'N/A') 
            error_message = getattr(e, 'message', str(e))
            self.logger.error(f"Cloudflare API Error fetching zones: {e} (Code: {error_code}, Message: {error_message})")
            return []
        # Catch other potential Cloudflare errors
            ...
            self.logger.exception(f"An unexpected error occurred while fetching zones: {e}")
            return []
    ...
    async def records(self) -> List[Endpoint]:
        ...
        for zone in zones:
            ...
            try:
                ...
                dns_records_iterator = self.cf.dns.records.list(zone_id=zone_id, per_page=100)
                ...
                for record in dns_records:
                    ...
                    record_ttl = getattr(record, "ttl", None) # Default handled by Endpoint
                    record_proxied = getattr(record, "proxied", False) # Default handled by Endpoint

                    if not all([record_type, record_name, record_content]):
                        self.logger.warning(f"Skipping record object due to missing type, name, or content: {record}")
                        continue
            ...
            # Use cloudflare.APIError for specific API errors
                 ...
                 error_code = getattr(e, 'code', 'N/A')
                 error_message = getattr(e, 'message', str(e))
                 self.logger.error(f"Cloudflare API Error fetching records for zone {zone_name}: {e} (Code: {error_code}, Message: {error_message})")
            # Catch other potential Cloudflare errors
            except cloudflare.CloudflareError as e:
                 self.logger.error(f"General Cloudflare Error fetching records for zone {zone_name}: {e}")
            except Exception as e:
                 self.logger.exception(f"An unexpected error occurred while fetching records for zone {zone_name}: {e}")
    ...
    async def _create_record(self, endpoint: Endpoint) -> None:
        ...
        try:
            for target in endpoint.targets:
                ...
                record_ttl = endpoint.record_ttl if endpoint.record_ttl is not None else 1
                ...
                self.logger.info(f"Creating DNS record: {endpoint.record_type} {record_data['name']} -> {record_data['content']} (TTL: {record_data.get('ttl', 'Auto')}, Proxied: {record_data['proxied']})" )
        ...
        # Use cloudflare.APIError for specific API errors
            ...
            error_code = getattr(e, 'code', 'N/A')
            error_message = getattr(e, 'message', str(e))
            self.logger.error(f"Cloudflare API Error creating DNS record for {endpoint.dnsname}: {e} (Code: {error_code}, Message: {error_message})")
        except cloudflare.CloudflareError as e:
             self.logger.error(f"General Cloudflare Error creating DNS record for {endpoint.dnsname}: {e}")
        except Exception as e:
            self.logger.exception(f"An unexpected error occurred while creating DNS record for {endpoint.dnsname}: {e}")
    
    async def _update_record(self, old_endpoint: Endpoint, new_endpoint: Endpoint) -> None:
        """
        ...
        if not zone_id:
            self.logger.error(f"Could not find zone ID for endpoint {new_endpoint.dnsname}")
            return
        ...
        if not record_id:
            ...
            self.logger.warning(f"Record ID not found for updating {old_endpoint.dnsname} ({old_endpoint.record_type}). Attempting to create instead.")
        ...
        try:
            ...
            if not new_endpoint.targets:
                self.logger.error(f"Cannot update record {new_endpoint.dnsname}: No targets specified.")
                return

            # Prepare record data according to v4 schema
            record_ttl = new_endpoint.record_ttl if new_endpoint.record_ttl is not None else 1 # Revert TTL logic (1=Auto)
            record_data = {
            ...
            self.logger.info(f"Updating DNS record: {record_id} ({old_endpoint.dnsname} -> {new_endpoint.dnsname}) Type: {new_endpoint.record_type}, Content: {record_data['content']}, TTL: {record_data.get('ttl', 'Auto')}, Proxied: {record_data['proxied']})" )
        ...
        # Use cloudflare.APIError for specific API errors
            ...
            error_code = getattr(e, 'code', 'N/A')
            self.logger.error(f"Cloudflare API Error updating DNS record {record_id} for {new_endpoint.dnsname}: {e} (Code: {error_code})")
        except cloudflare.CloudflareError as e:
             self.logger.error(f"General Cloudflare Error updating DNS record {record_id} for {new_endpoint.dnsname}: {e}")
        except Exception as e:
            self.logger.exception(f"An unexpected error occurred while updating DNS record for {new_endpoint.dnsname}: {e}")
    ...
    async def _delete_record(self, endpoint: Endpoint) -> None:
        ...
        zone_id = await self._get_zone_id_for_endpoint(endpoint)
        if not zone_id:
            self.logger.error(f"Could not find zone ID for endpoint {endpoint.dnsname} during deletion.")
            return
        
        # Get record ID
        record_id = await self._get_record_id(zone_id, endpoint)
        if not record_id:
            self.logger.warning(f"Could not find record ID for deleting endpoint {endpoint.dnsname} ({endpoint.record_type}). Skipping deletion.")
            return
        
        # Delete record
        try:
            # Include record type in the log message
            self.logger.info(f"Deleting DNS record: {endpoint.record_type} {endpoint.dnsname} (ID: {record_id})")
        ...
        # Use cloudflare.APIError for specific API errors
            ...
            error_code = getattr(e, 'code', 'N/A')
            error_message = getattr(e, 'message', str(e))
            self.logger.error(f"Cloudflare API Error deleting DNS record {record_id} for {endpoint.dnsname}: {e} (Code: {error_code}, Message: {error_message})")
        except cloudflare.CloudflareError as e:
             self.logger.error(f"General Cloudflare Error deleting DNS record {record_id} for {endpoint.dnsname}: {e}")
        except Exception as e:
            self.logger.exception(f"An unexpected error occurred while deleting DNS record for {endpoint.dnsname}: {e}")
    ...
    async def _get_zone_id_for_endpoint(self, endpoint: Endpoint) -> Optional[str]:
        ...
        for zone in zones:
            ...
            if zone_name == domain:
                ...
                self.logger.debug(f"Found matching zone ID: {zone_id} for domain {domain}")
                # Ensure cache consistency (though zones() should handle this)
                self.zone_id_cache[domain] = zone_id 
                return zone_id
            
        self.logger.warning(f"No matching zone found for domain: {domain} among managed zones.")
        return None
    ...
    async def _get_record_id(self, zone_id: str, endpoint: Endpoint) -> Optional[str]:
        ...
        try:
            ...
            for record in dns_records:
                 ...
                 if record_name == endpoint.dnsname and record_type == endpoint.record_type and record_id:
                    # Found the specific record
                    return record_id
            
            self.logger.debug(f"Did not find existing record ID for {endpoint.dnsname} ({endpoint.record_type}) in zone {zone_id}")
            return None
        # Use cloudflare.APIError for specific API errors
        except cloudflare.APIError as e:
             error_code = getattr(e, 'code', 'N/A')
             error_message = getattr(e, 'message', str(e))
             self.logger.error(f"Cloudflare API Error getting record ID for {endpoint.dnsname}: {e} (Code: {error_code}, Message: {error_message})")
             return None
        except cloudflare.CloudflareError as e:
             self.logger.error(f"General Cloudflare Error getting record ID for {endpoint.dnsname}: {e}")
             return None
        except Exception as e:
            self.logger.exception(f"An unexpected error occurred while getting record ID for {endpoint.dnsname}: {e}")
            return None
    ...
    def _extract_domain_from_hostname(hostname: str) -> Optional[str]:
        ...
        if len(parts) > 2:
            ...
            if parts[-2] + "." + parts[-1] in [
                "com.au", "co.uk", "co.nz", "co.za", "com.br", "com.mx"
            ]:
                return ".".join(parts[-3:])
