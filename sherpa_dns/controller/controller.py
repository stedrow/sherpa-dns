...
class Controller:
    ...
    def __init__(
        ...
        self._event_triggered_reconcile_task: Optional[asyncio.Task] = None # Task for debouncing
    ...
    async def run_reconciliation_loop(self) -> None:
        ...
        self.logger.debug(f"Reconciliation loop starting with interval {self.interval} seconds")
    ...
    async def run_once(self) -> None:
        ...
        try:
            ...
            plan = Plan(current_endpoints, desired_endpoints, policy="sync").calculate_changes()
            ...
            if potential_deletes and self.cleanup_on_stop:
                for endpoint_id, endpoint in potential_deletes.items():
                    ...
                    self.logger.debug(f"Ensured endpoint {endpoint_id} ({endpoint.dnsname}) is marked for delayed cleanup.")
                ...
                plan.delete = []
                 ...
                 self.logger.info(f"Identified {len(potential_deletes)} endpoints not in desired state, but cleanup_on_stop=False. Ignoring.")
            ...
            if plan.has_changes():
                ...
                self.logger.info(f"Applying changes: {len(plan.create)} creates, {len(plan.update_old)} updates")
            ...
            self.logger.error(f"Error in reconciliation: {e}", exc_info=True) # Add exc_info for better debugging
    ...
    async def process_cleanup(self) -> None:
        ...
        try:
             ...
             if pending_status:
                  self.logger.debug(f"Checking status of {len(pending_status)} endpoints pending deletion:")
                  for endpoint_id, remaining_time in pending_status.items():
                       if remaining_time > 0:
                            self.logger.debug(f"  - Endpoint ID {endpoint_id} will be eligible for deletion in {remaining_time:.1f} seconds.")
                       else:
                            # This case should be rare as get_eligible_for_deletion usually handles it first
                            self.logger.debug(f"  - Endpoint ID {endpoint_id} is overdue for deletion by {-remaining_time:.1f} seconds.")
             else:
                  self.logger.debug("No endpoints currently pending deletion.")
        except Exception as e:
             self.logger.error(f"Error getting pending cleanup status: {e}", exc_info=True)
        ...
        try:
            ...
            self.logger.debug(f"Found {len(eligible_ids)} endpoints eligible for deletion by tracker.")
            ...
            for endpoint_id in eligible_ids:
                 if endpoint_id in current_endpoints_map:
                      ...
                      self.logger.warning(f"Endpoint ID {endpoint_id} marked for deletion, but not found in current records. Already deleted?")
            ...
            if endpoints_to_delete:
                ...
                # Apply changes
                self.logger.debug(f"Applying deletion plan for {len(endpoints_to_delete)} endpoints after cleanup delay")
                await self.registry.sync(deletion_plan)
            else:
                 self.logger.debug("No existing endpoints matched the eligible IDs for deletion.")
    ...
    async def process_event(self, event: dict) -> None:
        ...
        if event_type in ["die", "stop", "kill"]:
            ...
            self.logger.debug(f"Container {container_short_id} stopped event ({event_type}) received.") 
            ...
            try:
                ...
                container_endpoints = [
                    endpoint for endpoint in desired_endpoints
                    if endpoint.container_id == container_id
                ]
                if container_endpoints:
                    self.logger.debug(f"Unmarking {len(container_endpoints)} endpoints for started container {container_short_id}")
                    ...
                    self.logger.debug(f"No desired endpoints found for started container {container_short_id} to unmark (might lack labels?).")
            except Exception as e:
                self.logger.error(f"Error fetching desired endpoints during start event processing for {container_short_id}: {e}", exc_info=True)
        else:
            self.logger.debug(f"Ignoring Docker event type '{event_type}' for container {container_short_id}")
            return # Don't reconcile on ignored events

        # --- Debounce Reconciliation Trigger --- 
        if self._event_triggered_reconcile_task and not self._event_triggered_reconcile_task.done():
            self.logger.debug(
        ...
        self.logger.info(f"Scheduling reconciliation due to event '{event_type}' for container {container_short_id} (after {self._debounce_delay}s delay)")
        self._event_triggered_reconcile_task = asyncio.create_task(self._run_once_debounced())
    ...
    async def _run_once_debounced(self):
        ...
        try:
            ...
            self.logger.debug("Running event-triggered reconciliation after debounce delay.")
            await self.run_once()
        except asyncio.CancelledError:
             self.logger.debug("Debounced reconciliation task cancelled.")
        except Exception as e:
             self.logger.error(f"Error during debounced reconciliation run: {e}", exc_info=True)
