...
class CleanupTracker:
    ...
    def __init__(self, delay: str = "15m"):
        ...
        self.logger.debug(f"Cleanup delay set to {self.delay} seconds ({self.original_delay_str})")
    ...
    def mark_for_deletion(self, record_id: str) -> None:
        ...
        if record_id not in self.pending_deletions:
            ...
            self.logger.info(f"Marked record {record_id} for deletion (eligible in {self.original_delay_str})")
        else:
    ...
    def get_eligible_for_deletion(self) -> List[str]:
        ...
        for record_id, timestamp in list(self.pending_deletions.items()):
            ...
            if elapsed >= self.delay:
                self.logger.debug(f"Record {record_id} eligible for deletion (elapsed: {elapsed:.2f}s >= delay: {self.delay}s)")
                eligible.append(record_id)
                del self.pending_deletions[record_id]
                self.logger.info(f"Record {record_id} is eligible for deletion")
            else:
                self.logger.debug(f"Record {record_id} still pending deletion (elapsed: {elapsed:.2f}s < delay: {self.delay}s)")
