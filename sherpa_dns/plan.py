...
class Plan:
    ...
    def __init__(self, current: List[Endpoint], desired: List[Endpoint], policy: str = "sync"):
        """
        Initialize a Plan.
    ...
    def calculate_changes(self) -> Changes:
        ...
        current_by_id: Dict[str, Endpoint] = {endpoint.id: endpoint for endpoint in self.current}
        ...
        if self.policy == "sync":
            ...
            for current_endpoint in self.current:
                if current_endpoint.id not in desired_ids:
                    self.logger.debug(f"Endpoint {current_endpoint.id} ({current_endpoint.dnsname}) identified as no longer desired.")
