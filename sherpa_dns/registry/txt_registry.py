...
class TXTRegistry:
        ...
        for zone in zones:
            ...
            try:
                ...
                for record in dns_records:
                    ...
                    if record_type == "TXT" and record_name is not None and raw_content is not None:
                        ...
                        processed_content = content_to_process # Default to stripped content

                        # Decrypt content if encryption is enabled for the registry
                        if self.encrypt_txt:
                            # Pass the unquoted content to the decryption function
                            processed_content = self._decrypt_txt_content(content_to_process)
                        ...
                        if processed_content:
                            txt_records[record_name] = processed_content
                             ...
                             if self.encrypt_txt and content_to_process.startswith("v1:AES256:"):
                                  self.logger.warning(f"Failed to decrypt TXT record content for {record_name}. Record might be ignored.")
            # Catch Cloudflare specific API errors first
            except cloudflare.APIError as e:
                 error_code = getattr(e, 'code', 'N/A')
                 error_message = getattr(e, 'message', str(e))
                 self.logger.error(f"Cloudflare API Error fetching TXT records for zone {zone_name}: {e} (Code: {error_code}, Message: {error_message})")
            # Catch other Cloudflare errors
            except cloudflare.CloudflareError as e:
                 self.logger.error(f"General Cloudflare Error fetching TXT records for zone {zone_name}: {e}")
            # Catch any other unexpected errors
            except Exception as e:
                # Log the full traceback for unexpected errors
                self.logger.exception(f"An unexpected error occurred while fetching TXT records for zone {zone_name}: {e}")
    ...
    def _parse_txt_content(self, txt_content: str) -> Dict[str, str]:
        ...
        parts = txt_content.split(",") # If txt_content = 'k1=v1,k2=v2', parts = ['k1=v1', 'k2=v2']
        for part in parts:
            if "=" in part:
                key, value = part.split("=", 1)
                parsed_content[key.strip()] = value.strip() # Strip potential whitespace
    ...
    async def _update_txt_record(self, old_endpoint: Endpoint, new_endpoint: Endpoint) -> None:
        """
